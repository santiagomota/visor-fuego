...
function(el, x) {
  ...
  function verifyPublishedBuild() {
    const embeddedBuildId = siteBuild && siteBuild.build_id ? String(siteBuild.build_id) : '';
    if (!embeddedBuildId || typeof window.fetch !== 'function') return;

    const manifestUrl = 'assets/site-build.json?cache_bust=' + Date.now();
    window.fetch(manifestUrl, {cache: 'no-store'})
      .then(response => {
        if (!response.ok) throw new Error('HTTP ' + response.status);
        return response.json();
      })
      .then(remote => {
        const remoteBuildId = remote && remote.build_id ? String(remote.build_id) : '';
        if (!remoteBuildId || remoteBuildId === embeddedBuildId) return;

        const url = new URL(window.location.href);
        if (url.searchParams.get('build') === remoteBuildId) return;
        url.searchParams.set('build', remoteBuildId);
        window.location.replace(url.toString());
      })
      .catch(error => {
        console.warn('No se pudo verificar el build publicado:', error);
      });
  }
  ...
  function updateFreshnessIndicator() {
    ...
    if (!siteDate) {
      ...
      html += freshnessRow(
        'Visor',
        'actualizado ' + relativeAge(siteDate),
        siteStatus,
        'Visor actualizado: ' + formatMadridTimestamp(siteDate)
      );
    }
    ...
    if (!firmsDate) {
      ...
      html += freshnessRow('FIRMS', 'última detección ' + relativeAge(firmsDate), firmsStatus, 'Última detección FIRMS: ' + formatMadridTimestamp(firmsDate));
    }
  ...
  panel.onAdd = function(map) {
    ...
    div.innerHTML =
      ...
      '<div class="fire-timeline" role="group" aria-label="Navegación temporal AEMET">' +
        '<button id="fire-layer-previous" type="button" title="Día anterior" aria-label="Día anterior">&#9664;</button>' +
        '<button id="fire-layer-play" type="button" title="Reproducir la secuencia temporal" aria-label="Reproducir la secuencia temporal">&#9654;</button>' +
        '<button id="fire-layer-next" type="button" title="Día siguiente" aria-label="Día siguiente">&#9654;</button>' +
