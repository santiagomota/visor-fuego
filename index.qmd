...
# Orden del selector AEMET en Leaflet. El paquete SIG clásico usa nombres
# down_YYYYMMDD...D00..D07, donde YYYYMMDD es fecha de emisión y Dxx el
# horizonte. En el catálogo ya se guarda valid_date = issue_date + D.
# Queremos que Península/Baleares aparezca antes que Canarias y que, dentro
# de cada zona, la primera capa sea la válida para la fecha de renderizado.
aemet_valid_date_rank <- function(date_chr, today = Sys.Date()) {
  d <- suppressWarnings(as.Date(date_chr))
  if (is.na(d)) return(999999L)
  if (identical(d, today)) return(0L)
...
map <- map |>
  ...
  hideGroup(c(effis_groups, "CCAA (NUTS2)", "Provincias (NUTS3)")) |>
  addControl(
  ...
  function setLayer(index) {
    ...
    if (layer.layer_kind === 'geojson') {
      fetch(layer.url)
        ...
        .then(data => {
          currentOverlay = L.geoJSON(data, { pane: 'aemetPane', style: function(feature) {
      ...
      currentOverlay = L.imageOverlay(layer.url, layer.bounds, { opacity: currentOpacity, interactive: false, pane: 'aemetPane' }).addTo(map);
