...
## v0.5.37 - Limpieza de fechas antiguas AEMET

- Limpia el directorio de extracción del paquete clásico AEMET antes de descomprimir.
- Conserva por defecto solo la emisión más reciente del paquete SIG.
- Limpia productos AEMET antiguos en `data/raw/aemet` antes de instalar los GeoTIFFs actuales.
- Evita incorporar ficheros huérfanos de `data/raw/aemet` al catálogo Leaflet salvo que `AEMET_INCLUDE_ORPHAN_RAW=true`.
- Limpia `assets/aemet` y `docs/assets/aemet` antes de regenerar capas web.
