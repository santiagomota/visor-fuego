...
detecciones térmicas recientes y las áreas quemadas en España.

**Versión actual:** `v0.6.32`
...
| [NASA FIRMS](https://firms.modaps.eosdis.nasa.gov/) | Detecciones térmicas VIIRS recientes | Normalización, ventanas de 6, 12, 24 y 48 horas, FRP y agrupación de alertas |
...
caché. El pipeline comprueba, antes y después del despliegue, que el HTML, el
...
Las ejecuciones principales de las 04:30 y 12:30 siempre ejecutan el pipeline
completo. GitHub Actions ejecuta además un watchdog ligero cada 20 minutos, a
los minutos `07`, `27` y `47`, entre las 04:00 y las 23:59. El watchdog consulta
el `site-build.json` realmente publicado:

- si Pages ya contiene el build correspondiente a la franja y AEMET y FIRMS
  están actuales, termina sin repetir el pipeline;
- si el build falta, está atrasado o no puede verificarse, ejecuta la
  actualización completa;
- si AEMET mantiene una emisión anterior o FIRMS no está fresco, vuelve a
  intentarlo cuando hayan pasado al menos 60 minutos desde el último build;
...
```text
.
├── R/                 # Funciones del pipeline y del visor
├── scripts/           # Descarga, preparación, validación y orquestación
├── assets/            # Snapshot canónico publicado y recursos runtime
├── data/processed/    # Productos procesados versionados
├── docs/              # Sitio renderizado localmente
├── *.qmd              # Páginas Quarto
├── _quarto.yml        # Configuración del sitio
└── sw.js              # Control de caché y actualización del navegador
```
...
En **Settings → Pages → Build and deployment**, la fuente debe ser **GitHub
Actions**. El workflow:
...
Los secretos necesarios en **Settings → Secrets and variables → Actions** son:
